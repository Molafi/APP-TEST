...
# defect. Remove once every plugin above targets 11+.
if key in src:
    print("    already set")
else:
    src = src.rstrip() + f"""

# Plugins that still target Kotlin jvmTarget 1.8 (flutter_timezone,
# flutter_tts, share_plus, speech_to_text) would otherwise fail the build
# against the template's Java 11. 1.8 bytecode is compatible with an 11
# target, so a warning is the right severity. Remove once they are updated.
{key}=warning
"""
    open(path, "w", encoding="utf-8").write(src)
    print("    Kotlin JVM-target validation set to warning")
PY
