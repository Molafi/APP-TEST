...
void main() {
  group('ErrorMapper.fromHttpStatus', () {
    ...
    test(
      '413 -> imageTooLarge',
      () => expect(
        ErrorMapper.fromHttpStatus(413).kind,
    ...
    test(
      '401 -> unauthenticated',
      () => expect(
        ErrorMapper.fromHttpStatus(401).kind,
    ...
    test(
      ...
      () => expect(
        ErrorMapper.fromHttpStatus(500).kind,
  ...
  group('ErrorMapper.fromException', () {
    test('SocketException -> noConnection', () {
      expect(
        ErrorMapper.fromException(const SocketException('x')).kind,
        AppErrorKind.noConnection,
      );
    });
    test('TimeoutException -> timeout', () {
      expect(
        ErrorMapper.fromException(TimeoutException('x')).kind,
  ...
  group('retryability', () {
    ...
    test(
      'invalidCredentials is not retryable',
      () => expect(
        const AppException(AppErrorKind.invalidCredentials).isRetryable,
        isFalse,
      ),
    );
  });
